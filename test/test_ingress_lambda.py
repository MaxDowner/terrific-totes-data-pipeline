...
from src.ingress_lambda import lambda_handler

"""
lambda handler is one part of many
suggest keep as is, but remane fn to ingress lambda for ACTUAL lambda fn to call 
current fn needs a return value to make it easier to test
"""
...
def test_currency_data_query_returns_correct_scaffold(mock_query):
    ...
    for result in results:
        assert isinstance(result[0], expected_id_type)
        assert isinstance(result[1], expected_code_type)
        assert len(result) == 2
...
def test_staff_data_query_returns_correct_scaffold(mock_query):
 ...
 [20,
  ...
  'flavio.kulas@terrifictotes.com']]
    ...
    for result in results:
        assert isinstance(result[0], expected_id_type)
        assert isinstance(result[1], expected_first_name_type)
        assert isinstance(result[2], expected_last_name_type)
        assert isinstance(result[3], expected_department_name_type)
        assert isinstance(result[4], expected_location_type)
        assert isinstance(result[5], expected_email_address_type)
        assert len(result) == 6
...
def test_design_data_query_returns_correct_scaffold(mock_query):
 ...
 [535, 'Rubber', '/var/log', 'rubber-20240724-nndw.json']]
    ...
    for result in results:
        assert isinstance(result[0], expected_id_type)
        assert isinstance(result[1], expected_design_name_type)
        assert isinstance(result[2], expected_file_location_type)
        assert isinstance(result[3], expected_file_name_type)
        assert len(result) == 4
        assert result[2][0] == '/'
        assert "." in result[3]
...
def test_address_data_query_returns_correct_scaffold(mock_query):
 ...
 [30,
  ...
  '1083 286132']]
    ...
    for result in results:
        assert isinstance(result[0], expected_id_type)
        assert isinstance(result[1], expected_al1_type)
        assert isinstance(result[2], expected_al2_type)
        assert isinstance(result[3], expected_district_type)
        assert isinstance(result[4], expected_city_type)
        assert isinstance(result[5], expected_postal_code_type)
        assert isinstance(result[6], expected_country_type)
        assert isinstance(result[7], expected_phone_type)
        assert len(result) == 8
...
def test_counterparty_data_query_returns_correct_scaffold(mock_query):
 ...
 [20,
  ...
  '9621 880720']]
    ...
    for result in results:
        assert isinstance(result[0], expected_id_type)
        assert isinstance(result[1], expected_legal_name_type)
        assert isinstance(result[2], expected_al1_type)
        assert isinstance(result[3], expected_al2_type)
        assert isinstance(result[4], expected_district_type)
        assert isinstance(result[5], expected_city_type)
        assert isinstance(result[6], expected_postal_code_type)
        assert isinstance(result[7], expected_country_type)
        assert isinstance(result[8], expected_phone_type)
        assert len(result) == 9
...
def test_sales_data_query_returns_correct_scaffold(mock_query):
 ...
 [12834,
  ...
  10,
  50975,
  Decimal('2.48'),2,441,'2025-03-02','2025-03-02',18],
 ...
 [12837, 17, 2, 84063, Decimal('3.72'), 1, 210, '2025-03-02', '2025-03-01', 8]]
    ...
    # Assert
    for result in results:
        assert isinstance(result[0], expected_sales_order_id)
        assert isinstance(result[1], expected_staff_id)
        assert isinstance(result[2], expected_counterparty_id)
        assert isinstance(result[3], expected_units_sold)
        assert isinstance(result[4], expected_unit_price)
        assert isinstance(result[5], expected_currency_id)
        assert isinstance(result[6], expected_design_id)
        assert isinstance(result[7], expected_agreed_delivery_date)
        assert isinstance(result[8], expected_agreed_payment_date)
        assert isinstance(result[9], expected_agreed_delivery_location_id)
